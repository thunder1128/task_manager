...
- **ユーザー情報(自分のプロフィール)**: ヘッダーの「ユーザー情報」から、ログイン中のユーザーが自分の**ユーザー名**と**パスワード**を変更できる。パスワード変更は現在のパスワードの入力が必須で、変更すると他端末のセッションは失効する(今使っているセッションは維持)。重複名は不可。
...
| PUT | `/api/auth/me` | 自分のプロフィール更新。`username`(重複は409) / パスワード変更は `current_password`+`new_password`(現在のパスワード不一致は403) |
