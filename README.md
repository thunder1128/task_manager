...
- **検索・絞り込み**: タイトル/説明の検索、優先度・カテゴリ・タグ・イテレーション・種別での絞り込み
...
タスクは `project_id`(所属プロジェクト)、`column_id`(所属するかんばん列)、`iteration_id`(数値=所属イテレーション / `null`=プロダクトバックログ)、`type_id`(数値=種別 / `null`=種別なし)を持つ。
